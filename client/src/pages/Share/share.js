...
const Share = ({ handleBackdropOpen, setCurrentId }) => {
  ...
  const { sharedPlan, loading } = useSelector((state) => state.weeklyPlans);
  ...
  return (
    ...
    >
      ...
      <Grid item container sx={{ width: "75vw", my: 10 }}>
        ...
        <Grid item container>
          {sharedPlan ? (
            ...
            >
              ...
              <Typography>
                The plan does not exist or have been deleted!
              </Typography>
            </Grid>
          )}
        </Grid>
        ...
        >
          {sharedPlan &&
          sharedPlan[0].creator !==
            (user.authData.result._id || user.authData.result.googleId) ? (
            <Button variant="contained" onClick={() => handleAddToMyPlan()}>
